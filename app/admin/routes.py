    ...
    if request.method=='POST':
        ...
        for k in ('units','payment_methods'):
            ...
            if k=='units':
                ...
                values=[canonical_unit(v) for v in values]
                from app.models.commercial import Material
                used={m.unit for m in Material.query.all()}
                if not used.issubset(set(values)): raise ValueError('Conserve las unidades de materiales existentes; no se convierten cantidades automáticamente.')

...
@bp.get('/carrito')
def cart():
    return render_template('cart.html', delivery_options=DELIVERY)
...
def preview():
    try:
        ...
        return render_template('error.html', message='No se pudo leer el carrito. Vuelve a revisarlo e intenta de nuevo.'), 400
    ...
    return render_template('quote_preview.html', message=message, links=links, long_message=len(links[0][1]) > 7000)
